...
export default function WeatherStatCard({
  ...
  return (
    <Card
      className="Weather_StatsCard relative overflow-hidden w-full min-w-[12rem] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xl bg-white/80 dark:bg-gray-900/80 transition-transform duration-300 ease-in-out flex flex-col items-center hover:scale-105 hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-teal-300/50 group"
      tabIndex={0}
    >
      ...
      >
        <Icon className={`${iconClass} transition-transform transform duration-300`} aria-hidden="true" />
      </motion.div>

      {/* Title */}
      <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 mb-1 text-center drop-shadow-sm">
        {title}
      </h3>

      {/* Value */}
      <span className="text-3xl font-extrabold text-teal-800 dark:text-teal-200 mb-1 drop-shadow-sm font-sans text-center">
        {value}
      </span>

      {/* Description */}
      <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-1 px-2">
        {description}
      </p>
    </Card>
  );
